...
with(peak_spawning, plot(ice_off_anomaly, julian_day))
...
with(start_spawning, plot(ice_off_anomaly, julian_day))
...
# Plot Walleye peak spawning
```{r, eval = F}
smooth <- smooth_estimates(spawning_model_peak)
sm <- smooth %>%
  filter(smooth == "s(ice_off_anomaly)" ) %>%
  add_confint() %>%
  add_constant(coef(spawning_model_peak)["(Intercept)"]) %>% 
  mutate(upper_ci = upper_ci + coef(spawning_model_peak)["(Intercept)"],
         lower_ci = lower_ci + coef(spawning_model_peak)["(Intercept)"])
  
temp_residuals <- spawning_model_peak$model %>% 
  add_partial_residuals(spawning_model_peak) %>%
  add_constant(coef(spawning_model_peak)["(Intercept)"], column = 4) 
...
fig <- sm %>% filter(smooth == "s(ice_off_anomaly)" ) %>%
    ...
    theme_classic() +
    labs(y = "Walleye Peak Spawning DOY", x = "Ice-Off Anomaly") +
