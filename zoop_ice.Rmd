...
early.anomaly.plot <- feiner.df %>%
  ...
  geom_vline(aes(xintercept = as.Date(106.0, origin = as.Date("2018-01-01")),
                 color = "Walleye Spawning"), linewidth = 1) + 
  #Plot where daphnia and cyclopoid peaks occur
  geom_segment(data = peaks %>% filter(name == "Cyclopoid" & ice_off_anomaly == early.anomaly),
             aes(x = as.Date(JULIANDAY, origin = as.Date("2018-01-01")), y = count,
                 xend = as.Date(JULIANDAY, origin = as.Date("2018-01-01")), yend = -Inf), 
             color = "#A52A2A") +
...
average.anomaly.plot = feiner.df %>%
  ...
  geom_vline(aes(xintercept = as.Date(green_mean_ice_off, origin = as.Date("2018-01-01")), 
                 color = "Ice-Off"), linewidth = 1.5) +
  geom_vline(aes( xintercept = as.Date(113.9, origin = as.Date("2018-01-01")), 
...
late.anomaly.plot = feiner.df %>%
  ...
  geom_vline(aes(xintercept =  as.Date(green_mean_ice_off + late.anomaly, origin = as.Date("2018-01-01")), 
                 color = "Ice-Off"), linewidth = 1.5) +
  geom_vline(aes(xintercept = as.Date(124.2, origin = as.Date("2018-01-01")), 
                 color = "Walleye Spawning"), linewidth = 1) + 
  #Plot where daphnia and cyclopoid peaks occur
...
```{r}
subset.labels = c("Bosmina", "Calanoid", "Ceriodaphnia", "Chrysophyte", "Chydorus", "Cryptomonad", 
