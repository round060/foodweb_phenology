...
# Model walleye year classes by ice in first year
```{r}
# Final model!
gn.ice.gam <- gam(count ~ s(est.age) + s(ice_off_anomaly) + s(jd) + s(log(acres)) +
                    s(x, y) + s(lake.id, bs = "re") + s(year_f, bs = "re") +
                    offset(log(total.effort.1)), 
                  family = nb(link = "log"), 
                  select = T, method = "REML", data = walleye_anomaly_ice)
...
#k.check(gn.ice.gam)
#
gratia::appraise(gn.ice.gam)
#
gratia::draw(gn.ice.gam, select = 2)
#gratia::draw(gn.ice.gam)
```


# Plot Walleye gill net recruitment
```{r}
smooth <- smooth_estimates(gn.ice.gam)
sm <- smooth %>%
  filter(smooth == "s(ice_off_anomaly)" ) %>%
  add_confint() %>%
  add_constant(coef(gn.ice.gam)["(Intercept)"]) %>% 
  mutate(upper_ci = gn.ice.gam$family$linkinv(upper_ci + coef(gn.ice.gam)["(Intercept)"]),
         lower_ci = gn.ice.gam$family$linkinv(lower_ci + coef(gn.ice.gam)["(Intercept)"])) %>%
  transform_fun(inv_link(gn.ice.gam)) 
  
temp_residuals <- gn.ice.gam$model %>% 
  add_partial_residuals(gn.ice.gam) %>%
  add_constant(coef(gn.ice.gam)["(Intercept)"], column = 12) %>%
  transform_fun(inv_link(gn.ice.gam), column = 12) %>%
  filter(`s(ice_off_anomaly)` < 2) 
...
data_to_predict <- walleye_anomaly_ice %>% 
  select(est.age, ice_off_anomaly, jd, acres,x, y, total.effort.1) %>%
...
for (age in 2:12){
  newd <- data.frame(ice_off_anomaly = c(-17.52, 0, 14), acres = data_to_predict$acres,
                     x = data_to_predict$x, y = data_to_predict$y, 
                     est.age = age, jd = data_to_predict$jd, total.effort.1 = data_to_predict$total.effort.1, 
                     lake.id = "10005900", year_f = 2010)
  ...
  temp <- data.frame(average = c(median(early.ice.off), median(normal.ice.off), median(late.ice.off),
                     ...
                     low = c(quantile(early.ice.off, na.rm = T, probs = 0.025),
                             quantile(normal.ice.off, na.rm = T, probs = 0.025),
                             quantile(late.ice.off, na.rm = T, probs = 0.025),
                             quantile(1 - (early.ice.off/normal.ice.off), na.rm = T, probs = c(0.025))*100,
                             quantile(1 - (early.ice.off/late.ice.off), na.rm = T, probs = c(0.025))*100,
                             quantile(early.ice.off - normal.ice.off, na.rm = T, probs = c(0.025)),
                             quantile(early.ice.off - late.ice.off, na.rm = T, probs = c(0.025))),
                     high = c(quantile(early.ice.off, na.rm = T, probs = 0.975),
                             quantile(normal.ice.off, na.rm = T, probs = 0.975),
                             quantile(late.ice.off, na.rm = T, probs = 0.975),
                             quantile(1 - (early.ice.off/normal.ice.off), na.rm = T, probs = c(0.975))*100,
                             quantile(1 - (early.ice.off/late.ice.off), na.rm = T, probs = c(0.975))*100,
                             quantile(early.ice.off - normal.ice.off, na.rm = T, probs = c(0.975)), 
                             quantile(early.ice.off - late.ice.off, na.rm = T, probs = c(0.975))), 
                     age = age, 
                     difference = c("early", "normal", "late", "percent-early-normal", 
                                    "percent-early-late", "early-normal", "early-late")
                     )
  df.results = rbind(df.results, temp)
}
# Average percent decrease (negative means increase)
normal.percent <- df.results %>% filter(difference == "percent-early-normal") %>% select(-difference)
colMeans(normal.percent)

late.percent <- df.results %>% filter(difference == "percent-early-late") %>% select(-difference)
colMeans(late.percent)


# Average fish per net
normal <- df.results %>% filter(difference == "normal") %>% select(-difference)
normal.ice.fish <- colSums(normal)/data_to_predict$total.effort.1

early <- df.results %>% filter(difference == "early") %>% select(-difference)
early.ice.fish <- colSums(early)/data_to_predict$total.effort.1

late <- df.results %>% filter(difference == "late") %>% select(-difference)
late.ice.fish <- colSums(late)/data_to_predict$total.effort.1
...
# Average decline in fish per net
early.vs.normal <- df.results %>% filter(difference == "early-normal") %>% select(-difference)
early.vs.late <- df.results %>% filter(difference == "early-late") %>% select(-difference)
# Early ice-off decline vs normal ice-off
colSums(early.vs.normal)/data_to_predict$total.effort.1
# Early ice-off decline vs late ice-off
colSums(early.vs.late)/data_to_predict$total.effort.1
```
...
walleye_anomaly_ice_lake_specific <- gn_spatial %>%
  ...
  select(lake.id, lake.name,  year, birth.year, est.age, sampling.method, 
         total.effort.1, count, cpue, 
         ice_off_anomaly, x, y, log_acres, shore_mi, non.fry, year_f, jd, effort.ident) %>% 
  filter(est.age >= 2) %>%
  filter(est.age < 13) %>%
  filter(non.fry <= 1)
...
```{r}
gn.ice.gam.lake.specific <- gam(count ~ s(est.age) + s(ice_off_anomaly, k = 5) + s(jd) + 
                                  s(log_acres) +
...
#gratia::draw(gn.ice.gam.lake.specific, select = 2)
...
sm <- smooth %>%
  ...
  add_confint()  %>%
  mutate(upper_ci = gn.ice.gam.lake.specific$family$linkinv(upper_ci),
         lower_ci = gn.ice.gam.lake.specific$family$linkinv(lower_ci)) %>%
...
fig
# Figure S2
ggsave("./figures/FigureS2.jpeg", width = 10, height = 10)
```



# Get estimates of decrease in year-class (lake-specific)
```{r}
...
```{r}
walleye_anomaly_ice_old <- gn_spatial %>%
  ...
  mutate(lake.id = as.factor(as.character(lake.id)),
         year_f = as.factor(as.character(year_f)), 
         age_f = as.factor(est.age), 
         non.fry = ifelse(is.na(non.fry), 0, non.fry),) %>%
  filter(non.fry <= 1) %>%
...
# Model GS
gn.ice.gam.old <- gam(count ~ s(ice_off_anomaly) + 
                        s(ice_off_anomaly, age_f, bs = "fs") + s(x, y) + 
                        s(jd) + s(log(acres)) +
                    s(lake.id, bs = "re") + s(year_f, bs = "re") + offset(log(total.effort.1)), 
                  family = nb(link = "log"), method = "fREML", data = walleye_anomaly_ice_old)
#k.check(gn.ice.gam.old)
#appraise(gn.ice.gam.old)
#summary(gn.ice.gam.old) # Deviance explained = 48
#draw(gn.ice.gam.old, select = 1)
#draw(gn.ice.gam.old, select = 2)

#write_rds(gn.ice.gam.old, "./models/gillnet.recruitment.anomaly.gam_fs.rds")
#gn.ice.gam.old <- read_rds("./models/gillnet.recruitment.anomaly.gam_fs.rds")
...
pred.df = data.frame(
  ...
  age_f = rep(2:12, each = 69),
        jd = 213, acres = 9.5,
        lake.id = "01006200", year_f = 2011, total.effort.1 = 6) %>%
  mutate(row = row_number(), age_f = as.factor(age_f))

fs <- fitted_samples(gn.ice.gam.old, data = pred.df, n = 1000, seed = 1024,
...
fv <- fitted_values(gn.ice.gam.old, data = pred.df)
...
```{r}
fig3A <- readRDS("./figures/fig3A.rds")
fig3B <- readRDS("./figures/fig3B.rds")
fig3C <- readRDS("./figures/fig3C.rds")
...
fig <- sm %>%   filter(smooth == "s(ice_off_anomaly,age_f)" ) %>%
    ggplot(aes(x = ice_off_anomaly, y = est, color = age_f, group = age_f)) +
    geom_point(aes(x = ice_off_anomaly, y = `s(ice_off_anomaly)`), 
               alpha = 0.1,
               data = temp_residuals) +
    geom_line(lwd = 1.5) +
