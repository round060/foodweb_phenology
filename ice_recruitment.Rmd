...
five.years <- efish_ice_anomaly %>% group_by(DOW) %>% count() %>% dplyr::filter(n >= 3)
efish_ice <- efish_ice_anomaly %>% dplyr::filter(DOW %in% five.years$DOW)
```
...
pred.df = data.frame(
  average_anomaly = seq(-18, 16, by = 0.5),
  x = -94.7, y = 46.1,
  julian_day_survey = 266, log_acres = 7.5,
  DOW = "56074701", year_f = 2011, total_effort_1 = 1.4) %>%
...
fig <- fs %>%
  ggplot() +
  geom_line(data = fv, aes(x = average_anomaly, y = fitted), size = 1.5) + 
...
jd_smooth <- sm %>%
  ...
  mutate(upper_ci = gam.ice.lake.specific$family$linkinv(upper_ci),
         lower_ci = gam.ice.lake.specific$family$linkinv(lower_ci)) %>%
